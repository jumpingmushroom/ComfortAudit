...
DLL="$ROOT/src/ComfortAudit/bin/Release/net472/ComfortAudit.dll"
...
ASM_VERSION=$(grep -oP '(?<=PluginVersion = ")[^"]+' "$ROOT/src/ComfortAudit/Plugin.cs")
...
echo "==> staging"
rm -rf "$STAGE"
mkdir -p "$STAGE/plugins/ComfortAudit"
cp "$DLL"                        "$STAGE/plugins/ComfortAudit/"
...
OUT="$ROOT/dist/ComfortAudit-$VERSION.zip"
rm -f "$OUT"
( cd "$STAGE" && zip -qr "$OUT" . )
